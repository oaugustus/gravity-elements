...
## 9. Verificação (recap da limitação de ambiente, Etapa 0/1)

Karma/ChromeHeadless não roda neste sandbox Cowork (sem Chrome ARM64 instalável) — mesma limitação documentada nas Etapas 0/1, sem mudança. Verificação real desta etapa segue a mesma mitigação: build isolado (lint/rollup/tailwind) + execução ao vivo no Chrome do usuário via `claude-in-chrome` (essencial para os componentes de interação não trivial listados no checkpoint da seção "Fluxo de trabalho" — Select/SelectMenu/InputMenu/Listbox, PinInput, Slider, InputDate/InputTime, FileUpload) ou jsdom para os demais, nunca só leitura de código. Recomendação, mesma da Etapa 0: rodar `npm test` localmente (Mac do Otávio) ou via CI pelo menos uma vez por marco relevante da etapa.
...
- [ ] Componente: ColorPicker (wrapper Pickr)
