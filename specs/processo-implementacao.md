...
7. **`bugfix.md` — canal separado para correção de bugs (regra a partir da Etapa 2)** — quando a verificação independente desta sessão encontra um bug real num componente já implementado (mesmo que ainda não tenha sido confirmado pelo Otávio como "fechado"), a instrução de correção vai num arquivo **separado**, `bugfix.md` (raiz do repo), **não** em `plantask.md`. `plantask.md` é reservado exclusivamente para a definição da tarefa corrente do TODO (o que implementar) e nunca é reescrito para carregar uma instrução de correção — isso evita perder/confundir a definição da tarefa em andamento quando uma correção pontual surge no meio do ciclo. Mesma lógica de sobrescrita do `plantask.md`: `bugfix.md` reflete só a correção pendente mais recente (não acumula histórico — isso vive na sub-linha de evidência da spec). Quando não há bug pendente, `bugfix.md` não precisa existir/ser tocado.
